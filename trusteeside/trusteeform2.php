  ...
  if ($_SERVER["REQUEST_METHOD"] != "POST") {
    ...
    if ($interactionid_trusteeside!=""){//firstagent has been created successfully
      ...
      if (count($allagentsstates["agents"])==2){//all two agents have been created successfully
        ...
        $investoroffer_now=getrandomoffer(3,$playerid,$sourcefiledir,$investorofferList);
      ...
      if (isset($_POST["trusteechoice"])){
        if(!empty($_POST["trusteechoice"])){
          ...
          header("Location:http://{$gameserveraddress}/mergedgame/trusteeside/investorreply.php?code=$finalrepay_binary");
...
<form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post">
  ...
  <select name="trusteechoice">
    <?php
      ...
      if ($trusteegetNum==0){
        $zerooffer=0;
        echo "<option value=$zerooffer>$zerooffer</option>";
      }else{
