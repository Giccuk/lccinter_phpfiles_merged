...
sub    $game_total=20;
    ...
    $investorofferList=array("0","1","5","7","9","10","11","13","19","20");
    #$investorofferList=array("0");
    #$listlen=count($investorofferList);
    #$randoffer_indx=array_rand(range(1,$listlen));
    #$investoroffer=(int)$investorofferList[$randoffer_indx];
...
//10.
  ...
  //11 provide random offer in different games
  function getrandomoffer($indx,$userid,$sourcedir,$list){
    ...
    $filename=$sourcedir."/"."{$userid}".'offerlist.csv';
    if ($indx==1) {
    ...
    fclose($file_r);
    if($indx==sizeof($investorofferList)){
