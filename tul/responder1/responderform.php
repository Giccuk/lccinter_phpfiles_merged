  ...
  if ($_SERVER["REQUEST_METHOD"] != "POST"){
    ...
    if ($interactionid_responderside!=""){//firstagent has been created successfully
        ...
        //$proposeroffer_now=$proposeroffer;
          $proposeroffer_now=getrandomoffer(1,$playerid,$sourcefiledir,$proposerofferList);
    ...
    if (empty($_POST["responderchoice"])) {
      ...
      echo '<span style="color:#FF0000;"> You did not choose. Please choose again. </span>';
        ...
        if (isset($_POST["responderchoice"])&&!empty($_POST["responderchoice"])){
          ...
          header("Location:http://{$gameserveraddress}/mergedgame/tul/responder1/proposerreply.php?code={$answer}");  
        }
    } 
  }
...
<form action="responderform.php" method="post">
  ...
  Thanks for waiting! The proposer has decided to offer £<?php echo $proposeroffer_now;?> to you. <br>Please choose one decision from two options below.<br><br> 

  <select name="responderchoice">
    <option value="accept<?php echo "@{$interactionid_responderside}@{$proposeroffer_now}";?>">Accept</option>
    <option value="reject<?php echo "@{$interactionid_responderside}@{$proposeroffer_now}";?>">Reject</option>
  </select>
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (empty($_POST["responderchoice"])) {
      ...
      if (isset($_POST["responderchoice"])&&!empty($_POST["responderchoice"])){
        ...
        #writeinfovalue($interactionid_responderside,$proposeroffer_now,"{$sourcefiledir}/resinfo.txt");
        #header("Location:http://{$gameserveraddress}/ultimategame/responder1/proposerreply.php?interid={$interactionid_responderside}&");
      }
    }
    */
?>
